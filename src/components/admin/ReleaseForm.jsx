...
export default function ReleaseForm({ initialData, artists, onSubmit, spotifyData }) {
  ...
  // Add a state variable to track if env variables are available
  const [envStatus, setEnvStatus] = useState({
    checked: false,
    usingFallback: false
  });

  // Check environment variable status on mount
  useEffect(() => {
    const checkEnvStatus = async () => {
      try {
        ...
        setEnvStatus({
          checked: true,
          usingFallback: !(data.spotify_id && data.spotify_secret)
        });
        
        if (!(data.spotify_id && data.spotify_secret)) {
          console.log('Warning: Using fallback Spotify credentials');
        }
      } catch (error) {
